 ...
 * d'une tortue à la Logo.
 * 
 * Cette interface peut être invoquée :<BR>
 * <UL>
 *    <LI>soit comme une applet dans le contexte d'une page html
 *    (charger par exemple la page testTurtle.htm avec un navigateur
 *    équipé d'une machine virtuelle java 2, ou utiliser l'application
 *    appletviewer).
 *    </LI>
 *    <LI>soit comme une application autonome (lancée par la commande
 *    java TurtleTrip.
 * </UL>
 * 
...
public class TurtleTrip extends JApplet {
	/** La tortue controlée par cette interface */
  ...
  /** création et positionnement des différents composants de l'interface
   * utilisateur.
   * @param container Le Container qui accueille les différents composants de l'interface
   * @param f Le frame qui affiche l'interface dans le cas d'une application standalone. C'est nécessaire
   * 					pour faire le pack qui fixe les dimensions des différents composants. Ce paramèetre est null
   ...
   */
	...
	private JSlider getDistance() {
		if(distance == null){
			...
			distance.setToolTipText("distance de déplacement");
  ...
  /**
	...
	/**
   *  exécution en mode application autonome (stand-alone application).
   *  l'application crée une fenêtre dans laquelle l'interface est affichée
  ...
  public static void main(String[] args) {
    ...
    // calcul automatique de la taille de la fenêtre en fonction de
    // son contenu et des layout managers utilisés
    f.pack();
    // affichage de la fenêtre
    // et lancement du thread de gestion des événements
    f.setVisible(true);
  }
